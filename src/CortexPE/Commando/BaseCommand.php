...
abstract class BaseCommand extends Command implements IArgumentable, IRunnable, PluginOwned {
	...
	/**
     * Get formatted argument names from the registered arguments.
     *
     * @return array An array of formatted argument names, e.g., ["[target]", "[amount]"]
     */
    protected function getFormattedArgumentNames(): array
    {
        $formattedArguments = [];
        foreach ($this->getArgumentList() as $argumentList) {
            foreach ($argumentList as $argument) {
                $formattedArguments[] = "[" . $argument->getName() . "]";
            }
        }
    ...
    // Function to format the command usage with arguments highlighted, using the FormatUtils
    protected function formatCommandUsageWithHighlight(string $missingArgument = null) : string
    {
        $baseCommand = "/" . $this->getName();
        $arguments = $this->getFormattedArgumentNames();
        
        return self::formatCommandUsage($baseCommand, $arguments, $missingArgument);
    }

	/**
     * Format command usage with arguments highlighted as they are filled.
     *
     * @param string $baseCommand The base command, e.g., "/pay"
     * @param array $arguments The arguments to format, e.g., ["[player]", "[amount]"]
     * @param string|null $missingArgument The argument that is missing, to be highlighted
     * @param string $separator The separator between arguments, default is a space
     * @return string Formatted command usage
     */
    public static function formatCommandUsage(string $baseCommand, array $arguments, ?string $missingArgument = null, string $separator = ' ') : string
    {
        $formattedArgs = [];
        $highlight = true;

        foreach ($arguments as $arg) {
            if ($arg === $missingArgument) {
                $highlight = false;
            }

            $formattedArgs[] = $highlight ? TextFormat::WHITE . $arg : TextFormat::DARK_GRAY . $arg;
        }

        return TextFormat::WHITE . $baseCommand . $separator . implode($separator, $formattedArgs);
    }
	 ...
	 */
	protected function getUuid(CommandSender $sender) : UuidInterface
	{
		return $sender instanceof Player ? $sender->getUniqueId() : null;
	}
